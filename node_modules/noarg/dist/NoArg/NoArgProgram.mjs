...
import { NoArgError } from "./NoArgError.mjs";
import { NoArgParser } from "./NoArgParser.mjs";
import ThrowExit from "../helpers/ThrowExit.mjs";
import { getArrayLengthStr } from "../utils/index.mjs";
...
import { CustomTable } from "../helpers/custom-table.mjs";
export class NoArgProgram extends NoArgParser {
    ...
    renderHelpIntro() {
        console.log(colors.cyan.bold(this.name), this.options.description
    ...
    renderHelpUsageIntro() {
        ...
        if (this.options.customRenderHelp?.helpUsageStructure) {
            return console.log(colors.cyan('$'), this.options.customRenderHelp?.helpUsageStructure);
        }
        const commandItems = [colors.dim(this.name)];
    ...
    renderHelpFlags(flags) {
        const optionData = Object.entries(flags)
            ...
            const optionName = '--' +
                ...
                (aliasString ? '\n ' + aliasString : '');
            const optionType = (schema instanceof TypeArray
                ? this.colors.type(schema.config.schema.name) +
                    `[${getArrayLengthStr(schema.config.minLength, schema.config.maxLength)}]`
                ...
                (schema.config.required &&
                    ...
                    ? ''
                    : '?');
            const enumValues = (schema instanceof TypeString || schema instanceof TypeNumber) &&
    ...
    renderHelp() {
        ...
        if (this.options.trailingArguments) {
            ...
            console.log(` These arguments are ${colors.red('ignored')} by the program and are passed as is.`);
            console.log('');
        }
        ...
        if (this.config.help) {
            ...
            console.log(' Use', colors.yellow('--help-usage'), 'or', colors.yellow('-hu'), 'flag to see how to use the program');
        }
    }
    renderUsageStructure() {
        console.log(colors.green('$'), this.colors.programs('programs'), this.colors.arguments('fixed-arguments'), this.colors.arguments('optional-arguments'), this.colors.arguments('list-arguments'), this.colors.flags('flags'), this.colors.trailingArgs('trailing-args'));
    ...
    renderUsageHowToUseOptions() {
        CustomTable({
            ...
            border: this.system.enableHelpBoxBorder,
        }, this.renderUsageUtils.tableGroup('string', 'string', '--string string', '--string=string'), this.renderUsageUtils.tableGroup('number', '100', '--number 100', '--number=100'), this.renderUsageUtils.tableGroup('boolean\n(true)', 'true', '--boolean', '--boolean true', '--boolean=true', '--boolean yes', '--boolean=yes', colors.dim("* Casing doesn't matter"), '--boolean YeS'), this.renderUsageUtils.tableGroup('boolean\n(false)', 'false', '--boolean' + this.system.booleanNotSyntaxEnding, '--boolean false', '--boolean=false', '--boolean no', '--boolean=no', colors.dim("* Casing doesn't matter"), '--boolean fAlSe'), this.renderUsageUtils.tableGroup('array\ntuple', "['value1', 'value2']", '--option value1 value2', '--option=value1 value2'));
    }
    ...
    renderUsageSystemConfiguration() {
        if (this.system.allowEqualAssign) {
            ...
            this.renderUsageUtils.printValid(colors.yellow('--option') + colors.blue('=') + 'value');
        }
        else {
            this.renderUsageUtils.printGroupHeader('Options with equal value is disabled');
            this.renderUsageUtils.printValid(colors.yellow('--option'), 'value');
            this.renderUsageUtils.printInvalid(colors.yellow('--option') + colors.blue('=') + 'value');
        }
        if (this.system.booleanNotSyntaxEnding) {
            ...
            this.renderUsageUtils.printValid(colors.yellow('--option' + this.system.booleanNotSyntaxEnding));
        }
        else {
            this.renderUsageUtils.printGroupHeader('Boolean not syntax ending is disabled');
            this.renderUsageUtils.printValid(colors.yellow('--option'));
            this.renderUsageUtils.printInvalid(colors.red('--option' + this.system.booleanNotSyntaxEnding));
        }
        if (this.system.allowDuplicateFlagForPrimitive) {
            ...
            this.renderUsageUtils.printValid(colors.yellow('--option'), colors.dim('value1'));
            this.renderUsageUtils.printValid(colors.yellow('--option'), colors.dim('value1'), colors.yellow('--option'), 'value2');
        }
        else {
            this.renderUsageUtils.printGroupHeader('Duplicate flags for primitive is disabled');
            this.renderUsageUtils.printValid(colors.yellow('--option'), 'value');
            this.renderUsageUtils.printInvalid(colors.yellow('--option'), 'value1', colors.yellow('--option'), 'value2');
        }
        if (this.system.allowDuplicateFlagForList) {
            this.renderUsageUtils.printGroupHeader('Duplicate flags for list is enabled');
            this.renderUsageUtils.printValid(colors.yellow('--option'), 'value1 value2', colors.yellow('--option'), 'value3');
            this.renderUsageUtils.printValid(colors.yellow('--option'), 'value1', colors.yellow('--option'), 'value2 value3');
            if (this.system.overwriteDuplicateFlagForList) {
                ...
                this.renderUsageUtils.printValid(colors.yellow('--option'), colors.dim('value1'), colors.yellow('--option'), 'value2 value3');
                this.renderUsageUtils.printValid(colors.yellow('--option'), colors.dim('value1 value2'), colors.yellow('--option'), 'value3');
            }
            else {
                this.renderUsageUtils.printGroupHeader('Overwrite duplicate flags for list is disabled');
                this.renderUsageUtils.printValid(colors.yellow('--option'), 'value1 value2 value3');
                this.renderUsageUtils.printValid(colors.yellow('--option'), 'value1', colors.yellow('--option'), 'value2');
            }
        }
        else {
